...
export default function LoginPage() {
  ...
  useEffect(() => {
      ...
      if (time - lastSpawn > 100) {
        ...
        setGoogleParticles(prev => {
          const updated = prev.map(p => ({
            ...p, x: p.x + p.velocityX, y: p.y + p.velocityY, life: p.life - 0.02,
          })).filter(p => p.life > 0)
          return [...updated, newParticle].slice(-20)
        })
      } else {
        setGoogleParticles(prev => prev.map(p => ({
          ...p,
          x: p.x + p.velocityX,
          y: p.y + p.velocityY,
    ...
    googleAnimRef.current = requestAnimationFrame(animate)
    return () => { if (googleAnimRef.current) cancelAnimationFrame(googleAnimRef.current) }
  }, [isGoogleHovered])
  ...
  useEffect(() => {
    if (!isTelegramHovered || !telegramRef.current) {
      ...
      if (telegramAnimRef.current) cancelAnimationFrame(telegramAnimRef.current)
      return
    }
    ...
    const animate = (time: number) => {
      if (time - lastSpawn > 100) {
        ...
        const newParticle: Particle = {
          ...
          y: Math.random() * (rect.height + 40) - 20,
          size: Math.random() * 4 + 2,
          hue: Math.random() * 40 + 190, // Голубые оттенки для Telegram
          velocityX: (Math.random() - 0.5) * 2,
          velocityY: (Math.random() - 0.5) * 2,
          life: 1,
        }
        ...
        setTelegramParticles(prev => prev.map(p => ({
          ...p,
          x: p.x + p.velocityX,
          y: p.y + p.velocityY,
          velocityX: p.velocityX + (Math.random() - 0.5) * 0.3,
    ...
    telegramAnimRef.current = requestAnimationFrame(animate)
    return () => { if (telegramAnimRef.current) cancelAnimationFrame(telegramAnimRef.current) }
  }, [isTelegramHovered])
        ...
        <div className="relative overflow-hidden rounded-3xl border border-purple-500/20 bg-card/50 backdrop-blur-xl p-10 animate-fade-in-up">
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-4">
              ...
              {/* Google — оригинальный дизайн + частицы */}
              <div className="relative">
                <button
                  ...
                  className={`w-full flex items-center justify-center gap-3 py-4 px-6 rounded-xl border border-purple-500/20 bg-card/50 transition-all duration-300 text-muted-foreground hover:text-foreground hover:bg-purple-500/10 hover:border-purple-500/40 ${isGoogleHovered ? 'scale-[1.02] shadow-lg shadow-purple-500/20' : ''}`}
                >
              ...
              {/* Telegram — оригинальный дизайн + частицы */}
              <div className="relative">
                <a
                  ref={telegramRef}
                  ...
                  className={`w-full flex items-center justify-center gap-3 py-4 px-6 rounded-xl border border-purple-500/20 bg-card/50 transition-all duration-300 text-muted-foreground hover:text-foreground hover:bg-purple-500/10 hover:border-purple-500/40 ${isTelegramHovered ? 'scale-[1.02] shadow-lg shadow-purple-500/20' : ''}`}
                >
