...
export function Header() {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        ...
        // Фолбэк — проверяем localStorage (для Telegram auth в будущем)
  ...
  // Текст и ссылка кнопки зависят от авторизации
  const authButtonLink = isLoggedIn ? "/dashboard" : "/login"
  const authButtonText = isLoggedIn ? "Личный кабинет" : "Войти"
    ...
    >
      <div className="container mx-auto px-4 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <span
            className="text-xl font-bold bg-gradient-to-r from-purple-400 via-pink-400 to-purple-500 bg-clip-text text-transparent tracking-wider"
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          {/* Кнопка: Войти или Личный кабинет */}
          {!isChecking && (
      ...
      >
        <div className="container mx-auto px-4 py-6 flex flex-col gap-4">
          <button
            ...
            className="text-left text-foreground py-2 hover:text-purple-400 transition-colors"
          >
            Скачать
          </button>
          <button
          ...
          {/* Мобильная кнопка: Войти или Личный кабинет */}
          {!isChecking && (
            <Link
              href={authButtonLink}
